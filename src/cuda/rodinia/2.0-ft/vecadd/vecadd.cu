...
__global__ void vecaddKernel(int *v1_in, int *v2_in, int *v3_in, int *v_out, int size, int variable)
{
    int offset = size - variable;
    int idx = blockDim.x * blockIdx.x - offset;
    if (idx < size)
...
int main()
{
    ...
    vecaddKernel<<<NUMBLOCK, BSIZE>>>(d_vec1, d_vec2, d_vec3, d_out, NUMBLOCK * BSIZE, -2);
