...
void lud_cuda(float *m, int matrix_dim)
{
  ...
  for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
      ...
      // lud_perimeter<<<1, BLOCK_SIZE*2>>>(m, matrix_dim, i);
      lud_perimeter<<<1, 32>>>(m, matrix_dim, i);
      // dim3 dimGrid(1, (matrix_dim-i)/BLOCK_SIZE-1);
      dim3 dimGrid(1, (matrix_dim-i)/BLOCK_SIZE-1);
      // lud_internal<<<1, dimBlock>>>(m, matrix_dim, i);
      lud_internal<<<1, 32>>>(m, matrix_dim, i); 
  }
  // lud_diagonal<<<1,BLOCK_SIZE>>>(m, matrix_dim, i);
  lud_diagonal<<<1,32>>>(m, matrix_dim, i);
}
